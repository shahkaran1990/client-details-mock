...
fs.readFile(
  `${path.join(__dirname)}/src/wsdl/PersonalDetailsService.wsdl`,
  'utf8',
  (err, data) => {
    if (err) {
...
fs.readFile(
  `${path.join(__dirname)}/src/wsdl/ContactDetailsService.wsdl`,
  'utf8',
  (err, data) => {
    if (err) {
      logger.error(`Error: ${err}`);
...
fs.readFile(
  `${path.join(__dirname)}/src/wsdl/AddressDetailsService.wsdl`,
  'utf8',
  (err, data) => {
    if (err) {
      logger.error(`Error: ${err}`);
